...
@interface PTCallKitManager() <CXProviderDelegate>
@property (nonatomic, strong) CXProvider *provider;
@property (nonatomic, strong) CXCallController *callController;
@property (nonatomic, copy) ADCallKitActionNotificationBlock actionNotificationBlock;
@end

@implementation PTCallKitManager

static const NSInteger PTDefaultMaximumCallsPerCallGroup = 1;
static const NSInteger PTDefaultMaximumCallGroups = 1;

- (CXProviderConfiguration *)providerConfiguration{
    NSString *localizedName = @"微话";
...
+ (PTCallKitManager *)sharedInstance {
    static PTCallKitManager *instance = nil;
...
- (void)setupWithAppName:(NSString *)appName supportsVideo:(BOOL)supportsVideo actionNotificationBlock:(ADCallKitActionNotificationBlock)actionNotificationBlock
{
    ...
    configuration.maximumCallGroups = PTDefaultMaximumCallGroups;
    configuration.maximumCallsPerCallGroup = PTDefaultMaximumCallsPerCallGroup;
...
- (NSUUID *)reportIncomingCallWithContact:(id<ADContactProtocol>)contact completion:(PTCallKitManagerCompletion)completion {
    ...
    [self.provider reportCallWithUUID:callUUID updated:callUpdate];
    return callUUID;
}
